...
chnks <- chunk(from=1, to=nrow(train_data), by=mRows, method='seq')
...
test_data <- read.csv.ffdf(file="./data/test",header=TRUE,
                            VERBOSE=TRUE,nrows=nRows, first.rows=mRows,next.rows=mRows,colClasses=test_colcl)
...
chnks <- chunk(from=1, to=nrow(test_data), by=mRows, method='seq')
